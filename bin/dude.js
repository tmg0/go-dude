...
program.command('push')
  ...
  .action(async (str, options) => {
    ...
    const dockerComposeFilePath = path.join(config.dockerCompose.path, config.dockerCompose.fileName)

    try {
      await ssh.connect({ ...config.ssh })
      const { stdout: yml } = await ssh.execCommand(`cat ${dockerComposeFilePath}`)
      const json = YAML.parse(yml)
      const { image } = json.services[name]
      await ssh.execCommand(`sed -i 's|${image}|${str}|g' ${dockerComposeFilePath}`)
